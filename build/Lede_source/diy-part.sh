...
EOF
sed -i 's/192.168.1.1/192.168.2.1/g' package/base-files/files/bin/config_generate                  # 改登录ip
...
cat feeds/luci/modules/luci-mod-admin-full/luasrc/view/admin_status/index.htm |grep Temperature    # 添加主页的CPU温度显示
...
sed -i 's/\/bin\/login/\/bin\/login -f root/' /etc/init.d/ttyd                                     # 自动登录ttyd
